...
def read_silver(spark):
    silver = os.getenv("SILVER_PATH","s3a://deltabucket/bronze/wholeCorp_delta")
    df = s.read.format("delta").load(silver)
...
def vevtorize(df):
    ...
    df = (df
        ...
        .withColumn("has_聯絡人", has("聯絡人"))
        .withColumn("has_電話", has("電話"))
        .withColumn("has_手機", has("手機"))
        .withColumn("log_資本額", F.log1p(F.col("資本額")))
        .withColumn("log_年營收", F.log1p(F.col("年營收")))
    )
    df = df.fillna({"log_資本額": 0})

    # Text
    text_col = F.coalesce(F.col("產品說明"), F.col("主要產品"))
    ...
    # Categorical
    cats = []
    for c in ["縣市名稱","類別","上市櫃_基本資料","主要出口市場"]:
    ...
    num_cols = [c for c in ["log_資本額","log_實收資本總額","公司年齡"] if c in df.columns]
    bin_cols = ["has_官網","has_聯絡人","has_電話","has_手機"]
